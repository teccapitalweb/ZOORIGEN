 ...
 *   - skip_shop_pay=true   → evita el popup "Confirma que eres tú" de Shop Pay
 *
...
function buildCheckoutURL(plan, email) {
  ...
  // return_to: URL a la que Shopify redirige después del pago exitoso
  const returnTo = encodeURIComponent('https://www.zoorigen.com/pages/club-suscripcion.html?paid=1');
  return `${baseURL}?channel=buy_button&skip_shop_pay=true&return_to=${returnTo}${emailParam}`;
}
